...
<div class="flex items-center justify-between mb-5 flex-wrap gap-3">
    ...
    <div class="flex gap-2">
        <a href="{{ route('crm.contacts.export-pdf') }}?{{ http_build_query(request()->except('page')) }}"
           target="_blank"
           class="inline-flex items-center gap-1.5 px-3 py-2 border border-slate-300 text-slate-600 rounded-lg text-sm font-semibold hover:bg-slate-50 transition">
            📄 Export PDF
        </a>
...
<form method="GET" action="{{ route('crm.contacts.index') }}" class="card mb-5 flex flex-wrap gap-3 items-end">
    ...
    <select name="sort" class="input" style="max-width:150px">
        <option value="">Highest Spend (default)</option>
