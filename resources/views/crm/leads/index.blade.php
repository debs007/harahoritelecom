...
{{-- Filter --}}
<form method="GET" action="{{ route('crm.leads.index') }}" class="card mb-5 flex flex-wrap gap-3 items-end">
  <input type="text" name="search" value="{{ request('search') }}" placeholder="Search leads…" class="input" style="max-width:200px">
  <select name="stage" class="input" style="max-width:150px">
  ...
  <div class="flex items-center gap-2">
    <div>
      ...
      <input type="date" name="date_from" value="{{ request('date_from') }}" class="input text-sm" style="max-width:145px">
    </div>
    <div>
      <label class="label text-xs">To</label>
      <input type="date" name="date_to" value="{{ request('date_to') }}" class="input text-sm" style="max-width:145px">
    </div>
  </div>
  <label class="flex items-center gap-1.5 text-sm font-medium text-slate-700 cursor-pointer">
    <input type="checkbox" name="auto_only" value="1" {{ request('auto_only')?'checked':'' }} class="rounded">
    Cart Leads Only
  </label>
